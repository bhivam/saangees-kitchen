...
export function isOrderingOpen(dateStr: string): boolean {
  ...
  if (dateStr > todayStr) return true; // future days are always open
  if (dateStr < todayStr) return false; // past days are always closed
  // Same day: open only before the cutoff hour
